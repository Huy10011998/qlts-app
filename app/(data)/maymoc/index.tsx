...
export default function MayMocScreen() {
  const [maymoc, setMayMoc] = useState<Record<string, any>[]>([]);
  ...
  const handlePress = async (item: Record<string, any>) => {
    try {
      ...
      console.error(error);
      Alert.alert("Lỗi", "Không thể tải chi tiết máy móc");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLoadMore = () => {
    if (maymoc.length < total && !isLoadingMore) {
      fetchData(true);
    }
  };
